...
async function parseJson<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(err.error ?? err.error_code ?? `API error ${response.status}`)
  }
  return payload
}
...
export async function runMakeupVirtualTryOn(input: {
  ...
  const src_file_url = await ensurePublicImageUrl(input.imageSource, input.userId ?? 'guest')
  ...
  if (!isMakeupApiConfigured()) {
    ...
    return {
      mode: 'demo' as const,
      resultUrl: src_file_url,
      downloadUrl: src_file_url,
      payload,
    }
  }
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    ...
    const statusResponse = await parseJson<TaskResponse>(
      await fetch(`${API_BASE}/s2s/v2.0/task/makeup-vto/${encodeURIComponent(taskId)}`, {
