export type MakeupTexture = 'matte' | 'satin' | 'shimmer' | 'gloss' | 'metallic'
...
export type MakeupEffect = {
  category: string
  enabled?: boolean
  pattern?: { name?: string; type?: string; curvature?: number; thickness?: number; definition?: number }
  shape?: { name?: string }
...
export type MakeupVtoTaskStatus = 'idle' | 'queued' | 'running' | 'processing' | 'success' | 'error'
